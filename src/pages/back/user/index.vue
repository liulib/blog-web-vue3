<template>
    <div class="userContainer">
        <a-button type="primary" class="addButton" @click="showAddUserModal(0, null)">新增用户</a-button>
        <a-table
            :columns="columns"
        ...
        >
            ...
            <template #ifManager="{ record }">
                ...
                <a-tag v-if="record.ifManager === 1" color="success">是</a-tag>
            </template>
            ...
            <template #operation="{ record }">
                ...
                >编辑</a-button>
                <a-button danger @click="showChangePwdModal(record.id)">修改密码</a-button>
            </template>
        </a-table>
        <a-modal v-model:visible="changePwdVisible" title="修改密码" width="300px">
            <a-form :model="changePwdPrams">
                <a-form-item
                    ...
                    has-feedback
                >
                    <a-input v-model:value="changePwdPrams.password" props="password" />
                </a-form-item>
            </a-form>
            <template #footer>
                <a-button
    ...
    setup() {
        ...
        const getUserListReq = async () => {
            try {
                ...
                // 赋值分页
                state.reqParams.page = state.pagination.current;
        ...
        const showAddUserModal = (type: number, editData: user | null) => {
            // 处理数据
            if (type) {
